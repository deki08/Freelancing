...
/**
 * Copyright (C) 2022 PIXOUS INNOVATIONS - All Rights Reserved You may use,
 * distribute and modify this code under the terms of the XYZ license, which
 * unfortunately won't be written for another century. Project : dc-service Date
 * : 2023-02-24 Developer : priyamal
 */
@Component
public class LicenseChecker implements CommandLineRunner {
	...
	public LicenseChecker(ShutdownEndpoint shutdownEndpoint, ConfigurationService configurationService,
			UserService userService, RoleService roleService, RolePermissionService rolePermissionService,
			PasswordEncoder passwordEncoder, RefValueService refValueService) {
...
//        }
	...
	private boolean checkLicense() {
		...
		UriComponents uriComponents = UriComponentsBuilder
				.fromHttpUrl("http://68.183.188.184/licence/diagnostic-center.json").queryParams(params).build();
		ResponseEntity<LicenseResponse> response = restTemplate.getForEntity(uriComponents.toUriString(),
	...
	private void init() {
		...
		Optional<MalasiyaEntry> malasiaya = entryService.find(1L);
		if (!malasiaya.isPresent()) {
			MalasiyaEntry malVale = new MalasiyaEntry();
			entryService.save(malVale);
		}
	}
}
