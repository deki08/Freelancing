...
public interface Actual_BillRepository extends JpaRepository<Actual_Bill, Long> {
	...
	@Query(value = "SELECT * FROM actual_bill  where " + "( name like %:#{#page.text}% OR " + " agency like %:#{#page.text}% OR "
			+ "patientId like %:#{#page.text}% OR " + "paid like %:#{#page.text}% OR " + "due like %:#{#page.text}% OR "
